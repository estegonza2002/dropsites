...
| ID | Requirement | P |
| --- | --- | --- |
| FR-86 | Generate a pre-built <iframe> embed snippet for every deployment | M |
...
| FR-115 | Inject loading="lazy" on all <img> tags at serve time — never modifies source files | M |
...
| ID | Requirement | P |
| --- | --- | --- |
| FR-117 | Infer human-readable page name from <title> tag of each HTML file | M |
| FR-118 | Fall back to first <h1> text if no <title> present | M |
| FR-119 | Fall back to filename (without .html extension) if neither <title> nor <h1> present | M |
...
| FR-400 | Publish a CLI tool: dropsites deploy <path> --slug <name> --workspace <id> | M·P2 |
...
| T-SRV-19 | Auto-nav widget — page titles from <title>, <h1>, filename fallback | Correct label shown for each page | Integration |
